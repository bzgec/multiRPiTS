def displayProcessScript(displayProcessFlag, printFlag_displayProcess, dhtArr, fanArr):
	...
	if variablesConfig.displayScript_sleepDuration_fanScript == True:
		variablesConfig.displayScript_sleepDuration = variablesConfig.fanScript_sleepDuration
		
	# subprocess.Popen('python3 /home/pi/scripts/mailSender.py displayScript.py Program started: ' + datetime.datetime.today().strftime('%Y/%m/%d %H:%M:%S'), shell=True, stdout=subprocess.PIPE, stdin=subprocess.PIPE)	
	try:
		while displayProcessFlag.value:
			...
			draw.text((x, y),    'D:' + str(fanArr[0]) + ', R:' + str(fanArr[1]) + ', ' + str(fanArr[2]),  font=font, fill=255)
			y += fontSize + linePadding
			draw.text((x, y),    'p:' + str(fanArr[4]),  font=font, fill=255)
			y += fontSize + linePadding
			draw.text((x, y),    'i:' + str(fanArr[5]) + ', pid sum:' + str(fanArr[7]),  font=font, fill=255)
			y += fontSize + linePadding
			draw.text((x, y),    'd:' + str(fanArr[6]) + ', delta_t:' + str(fanArr[8]),  font=font, fill=255)
			y += fontSize + linePadding

			draw.text((x, heightOLED-2*(fontSize+linePadding)),    'TEMP: ' + str(dhtArr[0]),  font=font, fill=255)
			draw.text((x, heightOLED-(fontSize+linePadding)), 'Humidity: ' + str(dhtArr[1]), font=font, fill=255)


			disp.image(image)
			disp.display()
			print('displayProcess: Runned for: {0:0.4}'.format(time()-runTime) + 's')
			print('displayProcess: Going to sleep for ' + str(variablesConfig.displayScript_sleepDuration) + 's\n')
